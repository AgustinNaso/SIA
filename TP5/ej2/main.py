...
saveFolderPath = f"/Users/gastondeschant/facultad/SIA-image-tp5/ej2/{config['images_folder']}/"
...
for i in range(len(x_train)):
    digit = x_decoded[i].reshape(images_shape[0], images_shape[1], colors_dim)
    print(x_test_encoded[i])
    figure = np.zeros((images_shape[0], images_shape[1], colors_dim))
    figure[:][:] = digit
    plt.figure(figsize=(1,1))
    plt.imshow(figure, cmap='Greys_r')
    plt.savefig(saveFolderPath+f"lego {i} gen")
    plt.clf()

# for i in range(10):
    asdf = x_test[i].reshape(figure[:][:].shape)#(60,80,3))
    figure[:][:] = asdf
    plt.figure(figsize=(1,1))
    plt.imshow(figure, cmap='Greys_r')
    plt.savefig(saveFolderPath+f"lego {i} real")
    plt.clf()
...
plt.imshow(figure, cmap='Greys_r')
plt.savefig(saveFolderPath+"new_legos")
plt.show()
