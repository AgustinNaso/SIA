...
def add_noise(images: np.ndarray, noise_coverage: float, noise_factor: float) -> np.ndarray:
    ...
    for i in range(images.shape[0]):
        ...
        #     apply = random.uniform(0, 1)
        #     noisy_input[i][j] = images[i][j]
        #     if apply < noise_coverage:
        #         noisy_input[i][j] = images[i][j] + random.uniform(0, noise_factor) * (1 if images[i][j] == 0 else -1)
    return noisy_set
